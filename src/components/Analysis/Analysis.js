...
function Analysis() {
  ...
  const submitHandler = async (e) => {
    ...
    if (state === "-" || percent === "-") {
      ...
      const api = `https://3582829dcd76.ngrok.io/predictpercentage?state=${state}&percent=${percent.substring(
        0,
        2
      )}`;
  ...
  return (
    <div className="analysis">
      ...
      <div className="analysis__butons">
        <button
      ...
      {pred > 0 && <div className="analysis__val">{pred}</div>}
    </div>
  );
}
